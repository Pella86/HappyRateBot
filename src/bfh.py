...
class BinaryFile:
    ...
    def write(self, dtype, data):
        ...
        log.debug("writing {}".format(dtype))
        self.file.seek(self.co)
        
        b = as_bytes(dtype, data)
        self.file.write(b)
        
        log.debug("written {} bytes".format(len(b)))
        self.co += len(b)
    ...
    def write_256hash(self, hash_id):
        self.file.seek(self.co)
        
        self.file.write(hash_id)
        
        self.co += 32
    
    def read_256hash(self):
        self.file.seek(self.co)
        hash_id = self.file.read(32)
        self.co += 32
        return hash_id
